...
from analyse_voted_channels_data.ideal_case_found_channels_with_covariance_w_neg_1 import (
    ideal_case_channels,
)


for dataset in ideal_case_channels.keys():
    for trigger in ideal_case_channels[dataset].keys():
        for model in ideal_case_channels[dataset][trigger].keys():
            clean_channels = ideal_case_channels[dataset][trigger][model]["clean"]
            poison_channels = ideal_case_channels[dataset][trigger][model]["poison"]

            dups = 0
            for item in poison_channels:
                if item in clean_channels:
                    dups += 1
            print(f"{dataset} {trigger} {model}: {dups}")
