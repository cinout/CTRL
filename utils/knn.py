...
def inject(poison, batch_data, trigger=None):
    ...
    for data_ in batch_data:
        if trigger is not None:
            data_[
                :, x_ : x_ + poison.trigger.shape[1], y_ : y_ + poison.trigger.shape[2]
            ] = trigger
        else:
            data_[
                :, x_ : x_ + poison.trigger.shape[1], y_ : y_ + poison.trigger.shape[2]
...
def knn_monitor(
    ...
    if subset:
        ...
        idx = [
            i
            for i, f in enumerate(memory_data_loader.dataset.dataset.imgs)
            if f[0].split("/")[-2] in class_list
        ]
        feature_labels = torch.tensor(
            memory_data_loader.dataset.dataset.targets, device=feature_bank.device
        )[idx]
    else:
        feature_labels = torch.tensor(
